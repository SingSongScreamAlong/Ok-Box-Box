...
export class VoiceSystem {
  ...
  private fallbackToKeyboardPTT(reason: string) {
    if (this.hidDevice) {
      try {
        this.hidDevice.close();
      } catch (e) { /* ignore */ }
  ...
  private startHIDButtonDetection() {
    ...
    const calibrationCycles = new Map<string, number>();
    const recentToggleTimes: number[] = [];
    const ignoredCalibrationKeys = new Set<string>();
    ...
    const resetCalibration = (reason: string, noisyKey?: string) => {
      calibrated = false;
      buttonByteIndex = -1;
      buttonBitMask = 0;
      candidateByte = -1;
