...
const irsdk_node_1 = require("irsdk-node");
...
function registerProtocol() {
    electron_1.app.setAsDefaultProtocolClient('okboxbox');
}
...
async function checkForUpdates(silent = true) {
    try {
        ...
        if (!isNewer) {
            if (!silent)
                await electron_1.dialog.showMessageBox({ type: 'info', title: 'Ok, Box Box', message: 'Relay is up to date.' });
            return null;
        }
        ...
        if (result.response === 0)
            await electron_1.shell.openExternal(info.download_url);
...
function connectToServer(accessToken) {
...
function startIRacingRelay() {
    ...
    try {
        iracing = new irsdk_node_1.IRacingSDK();
...
electron_1.ipcMain.on('open:website', () => {
    electron_1.shell.openExternal('https://okboxbox.com');
});
...
electron_1.app.whenReady().then(async () => {
    ...
    const savedVoiceSettings = store.get('voiceSettings');
    voiceSystem = new voice_1.VoiceSystem({
