            ...
            return (
              <tr key={e.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                <td className="py-3 px-3 text-white/70">
                  {e.user_display_name || 'ΓÇö'}
                  <div className="text-[10px] text-white/30 font-mono">{e.user_email || e.user_id || e.org_id || 'ΓÇö'}</div>
                </td>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-white/[0.06]">
            {['When', 'Action', 'Admin', 'Product', 'Old ΓåÆ New', 'Notes'].map(h => (
              <th key={h} className="text-left py-2.5 px-3 text-[10px] uppercase tracking-wider text-white/30 font-medium">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map(e => (
            <tr key={e.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
              ...
              <td className="py-2.5 px-3 text-white/60">{(e.metadata as any)?.product || 'ΓÇö'}</td>
              <td className="py-2.5 px-3 text-white/30 font-mono text-[10px]">
                ...
                {e.previous_status && e.new_status && <span className="text-white/20"> ΓåÆ </span>}
