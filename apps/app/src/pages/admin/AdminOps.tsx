                ...
                <tbody>
                  {sockets.length === 0 ? (
                    ...
                    <tr key={s.socketId} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="py-2.5 px-3 text-white/50">{s.surface || 'ΓÇö'}</td>
                      <td className="py-2.5 px-3">
          ...
          {traceId && (
            <div className="bg-white/[0.03] border border-white/[0.07] rounded p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {tracePolling && <span className="text-[10px] text-cyan-400 animate-pulse">ΓùÅ Collecting</span>}
...
function RoleBadge({ role }: { role: string }) {
  ...
  return <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${c}`}>{role || 'ΓÇö'}</span>;
}
