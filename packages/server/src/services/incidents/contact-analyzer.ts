...
export class ContactAnalyzer {
    constructor(private spatialAwareness?: SpatialAwarenessService) { }
    ...
    detectContact(trigger: IncidentTrigger): ContactDetection {
        ...
        // Real Analysis from World Model
        let realOverlap = 0.5;
        let realRelativePos: 'alongside' | 'ahead' | 'behind' = 'alongside';

        if (this.spatialAwareness) {
            const carA = this.spatialAwareness.getCarState(trigger.primaryDriverId);
            // Assuming first nearby driver is the contact partner
            const nearbyId = trigger.nearbyDriverIds[0];
            const carB = nearbyId ? this.spatialAwareness.getCarState(nearbyId) : undefined;

            if (carA && carB) {
                const gap = Math.abs(carA.lapDistPct - carB.lapDistPct) * 4000; // Est track length
                // Use Kinematics logic here if imported, or re-calc simple overlap
                if (gap < 4.5) {
                    realOverlap = (4.5 - gap) / 4.5;
                    realRelativePos = 'alongside';
                } else {
                    realOverlap = 0;
                    realRelativePos = 'behind'; // Fallback
                }
            }
        }
        ...
        const evidence: ContactEvidence = {
            ...
            overlapPercentage: realOverlap,
            timeToContact: 0,
            avoidabilityScore: 0.5,
            relativePosition: realRelativePos,
